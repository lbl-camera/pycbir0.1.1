...
import glcm, histogram,lbp,hog_rom,inception_feature_extraction
...
def searching(feature_vectors_database,feature_vectors_retrieval, similarity_metric,image_paths,retrieval_number,file,list_of_parameters,feature_extraction_method,path_database):
    ...
    file = path_database + "features/indexing" + "_" + feature_extraction_method + parameters_name +'_'+similarity_metric+'_'+str(retrieval_number)+'_nClusters_'+str(nClusters)+'.pickle'
    ...
    for cont1,i in enumerate(feature_vectors_retrieval):
        ...
        #create two arrays to storage the distances between the query image and the feature vectors, the second vector 'small_distance' has the 'distances' sorted
    ...
    result = []
    for cont1,i in enumerate(retrieval_distances):
        aux = []
        for j in i:
            aux.append(lists_of_image_paths[int(paths_cluster[cont1])][int(j)])
        result.append(aux)
...
def show_retrieval_indexing(images_retrieval,small_distances,k, path_database, feature_extraction_method, distance, folder_classes):
    ...
    fig.savefig(path_database + "results/result" + "_" + feature_extraction_method + "_" + distance + "_" + str(k) + "_indexing.png")   # save the figure to file   # save the figure to file
    #os.system(file)
